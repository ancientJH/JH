...
include("others.jl")
include("Assemble/Assemble_KR.jl")
include("Assemble/Assemble_R.jl")
include("Assemble/Assemble_u.jl")
include("Assemble/Assemble_φ.jl")
include("Assemble/Assemble_K.jl")
include("Solve/Newton.jl")
include("Solve/SCENIC.jl")
##########################################################
function Problem(solver, output, cellvalues_ϕ, cellvalues_u,
	...
	start_timestep = 1
    if isfile(joinpath(output_folder, "time_0_00950.jld"))  # 检查文件是否存在
		timestep, u, ϕ, states, states_old = load_results(output_folder, 0.00950)  # 从文件夹 "Shear" 加载时间步 0.001
	...
	for timestep in start_timestep:length(loadsteps)  #timestep in 0:n_timesteps
		...
		for newton_itr ∈ 1:solver.nitr_outer
			...
			if flag == "SCENIC"
				Δu = Masterstep_u!(u, ϕ, K_u, K_ϕ, R_u, R_ϕ, cellvalues_u, cellvalues_ϕ, dh, dh_u, dh_ϕ, bch, ch_u, ch_ϕ, Mat, states, states_old, flag_eta)
...
##########################################################
loadsteps = vcat(collect(0.0005:0.0005:0.01), collect(0.0102:0.0002:0.03))
