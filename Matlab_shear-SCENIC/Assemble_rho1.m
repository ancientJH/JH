function rho = Assemble_rho1(Sol_u, Sol_d, Coord, IEN, LM_u, LM_d, elementType, ...
    constitutive, body_force, traction, current_load_step, Psi_plus_rec, Psi_plus_old, FIndices, Kuu, Kud, Kdu, Kdd)
    ...
    FIndices = logical(FIndices);
    KUU = Ku(FIndices,FIndices);
    KUd = Kud(FIndices,active_indices_d);
    KdU = Kdu(active_indices_d,FIndices);
    % 主组装循环
    for ielem = 1:nElements
        ...
        h_min = compute_h_min(localCoord);
        k_uu1 = k_uu + h_min^2* eye(6);
        rho(ielem,1) = max(abs(eig(k_uu1\k_ud/k_dd*k_du)));
