...
function Problem(solver, output, cellvalues_ϕ, cellvalues_u,
	...
	output_folder = "Shear16037ele"
	if !isdir(output_folder)
		mkdir(output_folder)
	end
	start_timestep = 1
    if isfile(joinpath(output_folder, "time_0_00600.jld"))  # 检查文件是否存在
		timestep, u, ϕ, states, states_old = load_results(output_folder, 0.00600)  # 从文件夹 "Shear" 加载时间步 0.001
